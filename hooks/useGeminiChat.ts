...
import { chatbotSystemInstruction, personalInfo } from "../constants.ts";
    ...
    switch (contactFlowState) {
      ...
      case "collecting_message":
        ...
        submitContactForm(finalData)
          .then(() => {
            addBotMessage(
              `✅ Success! Your message has been sent to ${personalInfo.name}. They'll get back to you soon.`
            );
          })
          .catch((e) => {
            const errorMessage =
  ...
  const initializeChat = useCallback(() => {
    ...
    try {
      ...
      setMessages([
        {
          sender: "bot",
          text: `Hi! I'm ${personalInfo.name.split(' ')[0]}'s AI Twin. Feel free to ask me anything about ${personalInfo.name.split(' ')[0]}'s skills, projects, or experience.`,
        },
      ]);
      setContactFlowState("idle");
      setContactData({});
  ...
  const sendMessage = async (userMessage: string) => {
    ...
    try {
      ...
      if (botReply.trim() === "[START_CONTACT_FLOW]") {
        ...
        addBotMessage(
          `I can help with that! To send a message to ${personalInfo.name.split(' ')[0]}, what's your name?`
        );
      }
    } catch (e) {
      const errorMessage =
